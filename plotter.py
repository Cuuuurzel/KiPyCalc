...
class PlottingOptionPanel( Popup ) :
    ...
    def __init__( self, onConfirm ) :
        ...
        bgColor = ColorChooser( "Background Color :", [ 0, 0, 0, 100 ] )
        bgColor.size_hint = 1, 0.2
        plotColor = ColorChooser( "Axis Color :", [ 0, 100, 0, 100 ] )
        plotColor.size_hint = 1, 0.2
        axisColor = ColorChooser( "Plot Color :", [ 100, 100, 100, 100 ] )
    ...
    def dismiss( self, forced=False ) :
        if not forced : 
            try :
                ...
                Popup.dismiss( self ) 
                return { "axisColor" : self.colors[0].rgba(), \
                         "bgColor"   : self.colors[1].rgba(), \
                         "plotColor" : self.colors[2].rgba(), \
